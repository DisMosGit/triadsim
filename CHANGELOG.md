...
- Phase 4 is complete: a VLAN can be created over RESTCONF, `snmpwalk -v2c -c public <host>:161 1.3.6.1.2.1.17.4.3.1.2` returns the forwarding database as MAC to bridge port, and the STP state is readable at `/restconf/data/sim-l2-switching:stp/state`. Phase 3 delivered NETCONF (confirmed commit, notifications), Phase 2 the NETCONF base (`edit-config` → `commit` → `get-config`) and Phase 1 the SNMP walk (`ifDescr` plus the vendor RSSI OID) with `/metrics`. The radio and sync domain logic land in Phases 5–7. See [ROADMAP.md](ROADMAP.md).
